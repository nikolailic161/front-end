...
function getToken(role: 'korisnik' | 'admin'): string {
    const token = localStorage.getItem('api_token' + role);
    return 'Bearer ' + token;
}

export function saveToken(role: 'korisnik' | 'admin', token: string) {
    localStorage.setItem('api_token' + role, token);
}

function getRefreshToken(role: 'korisnik' | 'admin'): string {
    const token = localStorage.getItem('api_refresh_token' + role);
    return token + '';
}

export function saveRefreshToken(role: 'korisnik' | 'admin', token: string) {
    localStorage.setItem('api_refresh_token' + role, token);
}

async function refreshToken(role: 'korisnik' | 'admin'): Promise<string | null> {
